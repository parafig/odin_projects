...
const toDo = (function() {
    ...
    const showNewItem = (item) => {
        ...
        const todoContent = getContent();
        todoContent.append(todoItems)
    }
    
    const EventAddNewitem = (e) => {
        e.preventDefault();

        const {title, description, date, priority} = e.target;
        const todoItem = createNewITem(title.value, description.value, date.value, priority.value);
        showNewItem(todoItem);
        removeForm();
        removeNewItemButton();
    }
    ...
    const createTodoForm = () => {
        ...
        todoItemForm.append(todoTitleInput, todoDescriptionInput, todoDuedateInput, todoPriorityInput, submitBtn)
    ...
    return {createTodoContent, getForm, getContent, removeForm, createNewITem, showNewItem, createNewItemButton, removeNewItemButton, createTodoForm};
})()
...
todoContent.addEventListener('click', EventStartNewTodoItem);
